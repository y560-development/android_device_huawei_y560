...
Return<void> RadioImpl::getDeviceIdentity(int32_t serial) {
	...
	dispatchVoid(serial, mSlotId, RIL_REQUEST_GET_IMEISV);
    return Void();
}
...
int radio::getDeviceIdentityResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
		...
		if (response != NULL) {
			if (!ImeiSv && responseLen == 3) {
			ImeiSv = (char *) malloc(responseLen);
			if (ImeiSv != NULL) strcpy(ImeiSv, (char *) response);
			return 0;
			} else if (!Imei && responseLen == 17) {
			Imei = (char *) malloc(responseLen);
			if (Imei != NULL) strcpy(Imei, (char *) response);
			}
		}		
        if (response == NULL || (ImeiSv == NULL && Imei == NULL)) {
